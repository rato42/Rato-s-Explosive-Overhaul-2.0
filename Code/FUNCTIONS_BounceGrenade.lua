function ratonade_bounce(self, trajectory, attack_args, explosion_pos, bounces)
	...
	if explosion_pos then
		colide = colide_obj and (terrain_material or
							         ((not colide_obj:IsKindOf("SlabWallDoor") or colide_obj:IsDead() or colide_obj.pass_through_state ~=
												         "open"))) -- and IsKindOf(colide_obj, "WallSlab")--CollideSegmentsNearest(explosion_pos, explosion_pos:SetTerrainZ())
		if colide_obj then
	...
	if colide then
		...
		local col_obj_entity = colide_obj.class and EntityData[colide_obj.class] and EntityData[colide_obj.class].entity or
							                       false
		local surf_fx_type = terrain_material or col_obj_entity and col_obj_entity.material_type or
							                     GetObjMaterial(colide_obj_pos, colide_obj):match(":%s*(.+)$")
		...
		if start_high then
			...
			if ipt_bounce then
				...
				local closest_obj2, closest_pos2, closest_norm2 = GetClosestRayObj(ricochet_start_pos, bounce_pos, bounce_flags, 0,
				                                                                   function(obj, x, y, z)
		...
		if #bounce_trajectory > 0 then
			if #bounce_trajectory > 2 then
				DbgAddVector_rat(attack_args, bounce_trajectory[1].pos,
				                 (bounce_trajectory[3].pos - bounce_trajectory[1].pos) * 1000, const.clrPink)
			end
			DbgAddCircle_rat(attack_args, bounce_trajectory[1].pos, const.SlabSizeX / 5, const.clrRed)
			DbgAddVector_rat(attack_args, bounce_trajectory[1].pos,
			                 bounce_trajectory[#bounce_trajectory].pos - bounce_trajectory[1].pos, const.clrPaleYellow)
		end
...
function collision_obj_norm_pos(trajectory, attack_args, adj, traj_idx)
	...
	for i = trajectory_index, #trajectory do
		...
		if posnext then
			...
			local closest_obj, closest_pos, closest_norm = GetClosestRayObj(pos, ipt or ipt_posnext or posnext, bounce_flags, 0,
			                                                                function(obj, x, y, z)
				local window = IsBreakableWindow(obj) or nil
				local water = IsKindOf(obj, "WaterPlane") or nil
				if obj:IsKindOf("SlabWallDoor") then
