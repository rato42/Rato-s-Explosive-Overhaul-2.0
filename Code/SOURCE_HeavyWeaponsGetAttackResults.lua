function HeavyWeapon:GetAttackResults(action, attack_args)
	...
	local target_pos = attack_args.target_pos or lof_data and lof_data.target_pos or
						                   (IsValid(attack_args.target) and attack_args.target:GetPos())
	...
	local deviate
	if not attack_args.prediction and not attack_args.explosion_pos and self.trajectory_type ~= "bombard" then
	...
	elseif self.trajectory_type == "bombard" then
		...
		assert(false,
		       string.format("unknown trajectory type '%s' used in heavy weapon '%s' of class %s",
		                     tostring(self.trajectory_type), self.class, self.class))
	end

	if not attack_args.explosion_pos and
						((not trajectory or #trajectory == 0) and self.trajectory_type ~= "bombard" or not self.ammo or self.ammo.Amount <=
											0) then
		return {}
	end
	...
	local impact_pos = attack_args.explosion_pos or (trajectory and #trajectory > 0 and trajectory[#trajectory].pos) or
						                   target_pos
